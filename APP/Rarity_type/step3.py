...
def crop_middle(image_path, exclusive_bottom, gray_top):
    ...
    try:
        ...
        # Safety check for empty search area
        if y_max <= y_min:
            print(f"[STEP 3] Warning: Zero height crop detected (Y={y_min} to Y={y_max}). Using fallback.")
            return img[y_min : y_min + 30, :], False
        ...
        print(f"[STEP 3] Critical Error: {e}. Returning safe fallback.")
        # Return a small safe crop (30px tall) to prevent downstream failures
        safe_h = min(30, img.shape[0] - exclusive_bottom)
        return img[exclusive_bottom : exclusive_bottom + safe_h, :], False
